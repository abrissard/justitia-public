...
def construct_prompt(query: str, history: List[Dict[str, str]], context: List[Block]) -> List[Dict[str, str]]:
    ...
    source_prompt = "You are a helpful assistant knowledgeable about companies Terms and Conditions and Privacy Policies. " \
    ...
    for i, block in enumerate(context):
        block_str = f"[{chr(ord('a') + i)}] {block.title}\n{block.text}\n\n"
    ...
    question_prompt = f"In your answer, please cite any claims you make back to each source " \
                    ...
                    f"cite all of them. For example: \"Your data is protected [c, d, e]." \
                    f"If the name of the company is not mentioned in the user question, ask for clarification " \
                    f"before answering. \"\n\nQ: " + query
...
def talk_to_robot_internal(index, query: str, history: List[Dict[str, str]], k: int = STANDARD_K, log: Callable = print):
    try:
        ...
        yield {"state": "loading", "phase": "semantic", 'citations': [{'title': block.title, 'url': block.text} for block in top_k_blocks]}
